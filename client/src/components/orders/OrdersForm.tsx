...
export default function OrdersForm({
  ...
  const addProductionOrder = () => {
    setProductionOrdersInForm(prev => [...prev, {
  ...
  const updateProductionOrder = async (index: number, field: string, value: any) => {
    ...
    if (field === 'customer_product_id' || field === 'quantity_kg') {
      const prodOrder = productionOrdersInForm[index];
      const productId = field === 'customer_product_id' ? value : prodOrder.customer_product_id;
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
        ...
        <Form {...orderForm}>
          <form onSubmit={orderForm.handleSubmit(handleSubmit)} className="space-y-6">
            {/* Order Info Section */}
            ...
            <div className="border-t pt-6">
              ...
              <div className="space-y-4">
                {productionOrdersInForm.map((prodOrder, index) => (
                  <div key={index} className="p-4 border rounded-lg bg-gray-50" data-testid={`production-order-${index}`}>
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      <div className="col-span-1">
                        ...
                        >
                          <SelectTrigger className="h-auto min-h-[60px] w-full" data-testid={`select-product-${index}`}>
                        ...
                        <div>
                          ...
                          {quantityPreviews[index] && (
                            <div className="mt-2 p-3 bg-blue-50 rounded-lg border border-blue-200">
                              <div className="text-sm font-medium text-blue-800 mb-2">معاينة الكميات المحسوبة:</div>
                              <div className="grid grid-cols-2 gap-3 text-sm">
                                <div className="text-blue-700">
                                  <span className="font-medium">نسبة الزيادة:</span> {quantityPreviews[index].overrun_percentage}%
                                </div>
                                <div className="text-blue-700">
                                  <span className="font-medium">الكمية النهائية:</span> {quantityPreviews[index].final_quantity_kg} كيلو
                                </div>
                              </div>
                            </div>
                          )}
